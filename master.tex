...
\begin{titlepage}
    \begin{center}
        ...
        \large
        Zapiski s predavanj prof. Marjete Knez
...
%\noindent
%Hvala Emi, ki mi je pomagala s svojimi zapiski, in Tomažu, ki je pomagal s svojimi popravki.
%\rm\normalfont
%
%\clearpage
...
\begin{claim}
    ...
    \begin{equation*}
        f(x) - p(x) = \omega(x) [x_0, x_1, \dots, x_n, x] f = \omega(x) \frac{f^{(n+1)(\xi_x)}}{(n+1)!}
    \end{equation*}
    pri upoštevanju $\xi_x \in (a, b)$ in $ x \in [a, b]$.
    \begin{equation*}
...
\begin{figure}[H]
    \center
    \includegraphics[scale=0.25]{paradoks.jpg}
    \caption{Rungejev primer.}
\end{figure}
...
Lokalna napaka je reda $3$, globalna pa $2$.
    
\begin{figure}[H]
    \center
    \includegraphics[scale=1]{rk1.jpg}
    \caption{Princip delovanja ME metode}
\end{figure}

Postopek:
\begin{enumerate}
    \item odvod pri $x_n$
    \item dobimo vejo pri $x_n + \frac{1}{2} h$
    \item tangentno premico prestavimo v $x_n$
    \item dobimo približek v $x_n+1$
\end{enumerate}



\subsubsubsection{Heunova metoda}
...
\subsubsubsection{4-stopenjska RK metoda}
...
Lokalna napaka je reda 5, napaka metode pa je reda 4:
\begin{equation*}
    \max_{0 \leq n < m} |y(x_n) - y_n| = \mathcal{O}(h^4)
\end{equation*}
...
Če pri enočlenskih metodah želimo večji red metode, to plačamo s tem, da je treba izračunati več $k$-jev.
...
Najbolj znani razredi teh metod so:
\begin{itemize}
...
\begin{theorem}
    ...
    Potem za poljubno začetno matriko $Z_0 \in \R^{n \times p}$, $Z_0^T Z_0 = I$, zaporedje matrik $(Z_k)_k$ konvergira proti matriki,
...
Dobimo $\tilde{P}_1$, ki je $4 \times 4$ matrika. Iz nje sestavimo matriko $P_1$, za katero velja, da je $p_{1,1} = 1$, preostanek prve vrstice in prvega
stolpca pa je ničeln. Ostala polja zasede matrika $\tilde{P}_1$.
