...
%$n = 1$
%(slikca)
%$p(x) = f(x_0)\cdot 1 + [x_0, x_1] f (x-x_0) = f(x_0) + \frac{f(x_1) - f(x_0)}{x_1 - x_0}(x-x_0) -(x_1 \to x_0)->
%f(x_0) + \lim_{x_1 \to x_0} \frac{f(x_1)-f(x_0)}{x_1-x_0}(x-x_0)$
%\begin{align*}
...
%(imamo $k+1$ $x_i$-jev)
%
%Posplošitev rekurzivne formule.
%\begin{rem}
...
%    \begin{cases*}
%        \frac{f^{(k)}(x_i)}{k!} \text{, če } x_i = x_{i+1} = \dots = x_{i+k} \\
...
%    (trikotna shema)
%    \begin{equation}
%        p(x) = 1 \cdot 1 + 2 qcdot x + \frac{3}{2} x^2 - \frac{11}{2}x^3 + \frac{29}{2}x^3(x-1) - \frac{79}{8} x^3(x-1)^2
%    \end{equation}
%\end{example}
%
...
%\end{claim}
%
%Kako izbrati interpolacijske točke na $[a, b]$
%\begin{enumerate}
%    \item Ekvidistantne točke
%    $x_i = a + ih$, $h = \frac{b-a}{n}$. $i = 0, 1, \dots, n$
%    (slikca)
%    \item Čebiševe točke
%    Izbira, pri kateri je neskončna norma polinoma $\omega$ anjmanjša možna.
%    \begin{equation}
%        \min_{a \leq x_0 < x_1 < \dots < x_n \leq b} \norm{\omega}_{\infty, [a, b]} = \min_{a \leq x_0 < x_1 < \dots < x_n \leq b} \max_{x \in [a, b]} \right| \omega(x) \left|
%    \end{equation}
%    Rešitev so
%    \begin{equation}
%        x_i = \frac{a+b}{2} + \frac{b-1}{2}\cos(\frac{2i+1}{2n+2}\pi), i = 0, 1, \dots, n
%    \end{equation}
%\end{enumerate}
%
%Vprašanje:
%Recimo, da izberemo $f qin \mathscr{C}([a, b])$, izberemo zaporedje interpolacijskih točk $(\{x_0, x_1, \dots, x_n\})_n$ in
%večamo stopnjo $n$. Dobino zaporedje interpolacijskih polinomov $(p_n)_n$. Zanima nas, kaj se dogaja z napako
%\begin{equation}
%    \norm{f-p}_{\infty, [a, b]} -(n\to\infty)-> ??
%\end{equation}
...
%(Rungejev primer)
%
%$f(x) = \frac{1}{1-x^2}, [-5, 5]$ ekvidistantne točke. Napaka gre proti $\infty$ z večanjem $n$-ja.
%
%\subsubsection*{Odsekoma polinomske funkcije (zlepki)}
%
%IDEJA: Interval $[a, b]$ razdelimo na $m$ delov s stilnimi točkami $a = x_0 < x_q < \dots < x_m = b$
%(slikca)
%
%\begin{equation*}
%    s: [a, b] \to \R
...
%Polinom na $\right[ x_i, x_{i+1})$ določimo tako, da zadostimo pogojem
%\begin{align*}
%    p_i(x_i) =& f(x_i) \\
%    p_i'(x_i) =& f'(x_i) \\
%    p_i(x_[i+1]) =& f(x_{i+1}) \\
...
%
%\subsubsection*{Ideja za izpeljavo aproksimacijskih formul:}
%Kot približek za odvod funkcije $f$ v izbranem $x$ vzamemo vrednost odvoda interpolacijskega polinoma za $f$ na točkah $x_0, x_1, \dots, x_n$ pri tem
%izbranem $x$-u.
%Vemo že:
%\begin{equation*}
%    f(x) = p(x) + \omega(x)[x_0, x_1, \dots, x_n, x] f (\text{za } f \in \mathscr{C}^{n+1} ([a, b]))
%\end{equation*}
%\begin{equation*}
...
%\end{equation*}
%kjer prvi del imenujemo APROKSIMACIJA ODVODA, drugi del pa NAPAKA, ki jo označujemo z R(f) oziroma Rf.
%
%\begin{rem}
%    Odvod daljne diference (brez izpeljave):
%    \begin{equation*}
%        \frac{d}{dx} [x_0, x_1, \dots, x_n, x] f = [x_0, x_1, \dots, x_n, x, x] f
%    \end{equation*}
%\end{rem}
%
%Interpolacijske točke običajno izberemo EKVIDISTANTNO, za $x$ pa izberemo eno od interpolacijskih točk, na primer $x_k$, $k \in \{0, 1, \dots, n\}$.
%Tedaj dobimo
%\begin{equation*}
%    f'(x_k) = \sum_{i=0}^{n}f(x_i) \ell_{i, n}(x_k) + \omega'(x_k) [x_0, x_1, \dots, x_k] f + \omega(x_k) [x_0, x_1, \dots, x_k] f
%\end{equation*}
%kjer je prvi del približek, drugi napaka, tretji pa enak nič.
%\begin{equation*}
...
%    \end{equation*}
%    Tej formuli pravimo enostranska diferenca (samo prvi del).
%\end{ex}
%
%\begin{ex}
%    $n=2$, izberemo $x_0, x_1=x_0+h, x_2 = x_0+2h$. Zanima nas približek za $f'(x_1)$ in $f''(x_1)$.
%    \begin{equation*}
%        f(x) = f(x_0) \frac{(x-x_1)(x-x_2)}{(x_0-x_1)(x_0-x_2)} + f(x_1) \frac{(x-x_0)(x-x_2)}{(x_1-x_0)(x_1-x_2)} + f(x_2) \frac{(x-x_0)(x-x_1)}{(x_2-x_0)(x_2-x_1)} + (x-x_0)(x-x_1)(x-x_2) [x_0, x_1, x_2, x] f
%    \end{equation*}
%    Odvajamo:
%    \begin{equation}
...
%    \end{equation*}
%    \begin{equation*}
%        Rf = \omega'(x_1) [x_0, x_1, x_2, x] f = (x_1 - x_0)(x_1-x_2) \frac{f'''(\xi)}{3!} = - \frac{h^2}{6} f'''(\xi)
%    \end{equation*}
%    \begin{equation}
%        f'(x_1) = \frac{f(x_2) - f(x_0)}{2h} - \frac{h^2}{6} f'''(\xi)
%    \end{equation}
%    To je za prvi odvod. Še drugi:
%    \begin{equation*}
%        f''(x_1) = f(x_0) \frac{2}{2h^2} + f(x_1) \frac{2}{-h^2} + f(x_2) \frac{2}{2h^2} + (\omega(x) [x_0, x_1, x_2, x] f)'' |_{x=x_1}
%         = \frac{^2f(x_2) - 2 f(x_1) + f(x_0)}{h} + Rf
%    \end{equation*}
%    Tej se reče simetrična diferenca za drugi odvod.
%    \begin{equation*}
%        Rf = \omega''(x_1) [x_0, x_1, x_2, x_1] f + 2*\omega(x_1) [x_0, x_1, x_2, x_1, x_1] f + 0 
%    \end{equation*}
%    Preveri, da je $\omega''(x_1) = 0$ in $\omega'(x_1) = -h^2$.
%    
%    Sledi
%    \begin{equation*}
...
%    \end{equation*}
%    \begin{equation*}
%        f''(x_1) = \frac{^2f(x_2) - 2 f(x_1) + f(x_0)}{h} - \frac{1}{12} h^2 f^{(4)}(\xi)
%    \end{equation*}
%    (slikca)
%\end{ex}
%
...
%\end{itemize}
%
%Ko računamo vrednosti funkcije $f(x_i)$ zaradi daokrožitvenih napak aritmetike dobimo približek $\tilde{f}(x_i)$, da velja
%\begin{equation*}
%    \left| f(x_i) - \tilde{f}(x_i) \right| \leq \epsilon
%\end{equation*}
%
%\begin{ex}
%    \begin{equation*}
...
%    Sf = \int_{a}^{b} f(x) dx
%\end{equation*}
%$S: \mathscr{C}([a, b]) \to \R$, $S$ je linearen funkcional.
%
...
%
%IDEJA ZA IZPELJAVO FORMUL:
%Namesto funkcije $f$ integriramo interpolacijski polinom za $f$ na točkah $a \leq x_0 < x_1 < \dots < x_n \leq b$.
%
%Vemo že:
%\begin{equation*}
%    f(x) = p(x) + \omega(x) [x_0, x_1, \dots, x_n, x] f (za f \in \mathscr{C}^{n+1}([a, b])) (oznaka)
%\end{equation*}
%\begin{equation*}
%    p(x) = \sum_{i=0}^{n} f(x_i) \ell_{i, n}(x)
%\end{equation*}
...
%    Sf = šribližek za integral + napaka
%\end{equation*}
%Sf = Ff - Rf (integracijsko pravilo oz kvadraturna formula)
%
%\begin{equation*}
%    Ff = \int_{a}^{b} p(x) dx = \sum_{i=0}^{n} f(x_i) \int_{a}^{b} \ell_{i, n} (x) dx (=A_i - uteži) = \sum_{i=0}^{n} A_i f(x_i)
%\end{equation*}
%
%$x_0, x_1, \dots, x_n$ imenujemo VOZLI INTEGRACIJSKEGA PRAVILA.
%
%\begin{defn}
%    Red oziroma stopnja integracijskega pravila $Sf = Ff + Rf$ je enaka $m$, če je pravilo točno za vse polinome stopnje $\leq m$. To je,
%    če velja $Rp = 0$ za $\forall p \in \Pp_n$ in $R(x^{m+1}) \neq 0$. (ekvivalentno:)
%    \begin{equation*}
%        Rx^j = 0 ua j = 0, 1, \dots, m
%    \end{equation*}
%    (tole malo uredi, nekaj manjka)
%\end{defn}
...
\begin{itemize}
    \item Newton-Cotesova pravila
    Vozle izberemo ekvidistantno: $x_0 = a, x_i = x_0 + ih, h = \frac{b-a}{n}, i = 0, 1, \dots, n$
    Ločimo:
    \begin{itemize}
        ...
        \end{equation*}
    \end{itemize}
    \item Gaussova pravila
    Vozle izračunamo oziroma določimo tako, da je pravilo čim višjega reda.
\end{itemize}

\subsubsection*{Nekaj osnovnih Newton-Cotesovih (N-C) pravil}
\begin{ex}
    $n = 1$, $a = x_0$, $b = x_1 = x_0 + h$, $h = b-a$
    \begin{align*}
        \int_{a}^{b} f(x) dx =& \int_{a}^{b} (f(x_0) \frac{x-x_1}{x_0 - x_1} + f(x_1) \frac{x-x_0}{x_1-x_0}) dx + \int_{a}^{b} (x-x_0)(x-x_1) [x_0, x_1, x] f dx
                             =& f(x_0) \frac{1}{-h} \int_{x_0}^{x_1} (x-x_1) dx f(x_1) \frac{1}{h} \int_{x_0}^{x_1} (x-x_0) dx + \int_{x_0}^{x_1} (x-x_0) (x-x_1) [x_0, x_1, x] f dx
                             =& \frac{h}{2} (f(x_0) + f(x_1)) + Rf
    \end{align*}
    (pazi! nekaj komentarjev manjka (in drugih stvari))
    (*) Posplošen izrek o povprečni vrednosti
    Če $f$ na $[a, b]$ ne spremeni predzanaka, potem je 
    \begin{equation*}
        \int_{a}^{b} f(x) g(x) dx = g(\xi) \int_{a}^{b} f(x) dx, \xi \in [a, b]
    \end{equation*}
    Trapezno pravilo:
    \begin{equation*}
        \int_{x_0}^{x_1} f(x) dx = \frac{h}{2} (f(x_0) + f(x_1)) - \frac{1}{12} h^3 f''(\xi), \xi \in [x_0, x_1] za f \in \mathscr{C}^2([x_0, x_1])
    \end{equation*}
    ...
    \begin{itemize}
        ...
        \begin{equation*}
            Rp=? ua p\in \Pp_2
            R(x - x_0)^2 = S(\cdot - x_0)^2 + F(\cdot - x_0)^2 = \int_{x_0}^{x_1} (\cdot - x_0)^2 d\cdot - \frac{h}{2}((x_0 - x_0)^2 + (x_1 - x_0)^2) =
            = \frac{(x_1 - x_0)^3}{3} - \frac{h}{2} h^2 = \frac{h^3}{3} - \frac{h^3}{2} = - \frac{h^3}{6} \neq 0
        \end{equation*}
    \end{itemize}
\end{ex}
...
\begin{ex}
    ...
    \begin{equation*}
        \int_{x_0}^{x_1} f(x) dx = \int_{x_0}^{x_1} \frac{(x-x_1)(x-x_2)}{(x_0-x_1)(x_0-x_2)} dx + \int_{x_0}^{x_1} \frac{(x-x_0)(x-x_2)}{(x_1-x_0)(x_1-x_2)} dx + \int_{x_0}^{x_1} \frac{(x-x_0)(x-x_1)}{(x_2-x_0)(x_2-x_1)} dx + \int_{x_0}^{x_1} (x-x_0)(x-x_1)(x-x_2) [x_0, x_1, x_2, x] f dx
        A_0 = \frac{1}{h^2} \int_{x_0}^{x_1} (x-x_1)(x-x_2) dx = \frac{1}{h^2} \int_{x_0}^{x_1} h(t-1) h(t-2) h dt = \dots = \frac{h}{3}
    \end{equation*}
    Podobno:
    \begin{align*}
        A_1 = \frac{4}{3}h\\
    ...
    $R(x-x_0)^3 = S(x-x_0)^3 - F(x-x_0)^3 = \int_{x_0}^{x_2} (x-x_0)^3 dx - \frac{h}{3} ((x_0-x_0)^3 + 4(x_1-x_0)^3 + (x_2 - x_0)^3) =
    \frac{(x_2-x_0)^4}{4} - \frac{h}{3}(4h^3 + 8h^3) = (dopolni) = 0$
    Red je vsaj 3
    Za red = 4 nisem prepisoval - lahko ctrl+c
    Red je 3
    Uporabimo nastavek za napako:
    \begin{equation*}
        Rf = C f^{(m+1)} (\xi), \xi \in [a, b]
    \end{equation*}
    Za $f$ izberemo $f(x) = (x-x_0)^4$ in dobimo
    \begin{equation*}
        F(x-x_0)^4 = C * f^{(4)} (\xi) = C*4!
    \end{equation*}
    sledi
    \begin{equation*}
        C = -\frac{h^5}{90}
    ...
    \begin{rem}
        ...
        \begin{equation*}
            f(x) f(a) + f(a)'(x-a) + \frac{f''(a)}{2!} (x-a)^2 + \dots + (za m)
        \end{equation*}
        \begin{equation*}
            Rf = \dots
        \end{equation*}
...
\begin{equation*}
    A_0 = A_2 = h/3, A_1 = \frac{4h}{3}
\end{equation*}

\begin{ex}
    Primer odprtega N-C pravila
    \begin{equation*}
...
Poglejmo si neodstranljivo napako (napaka, do katere pride, ker ne računamo s točnimi vrednostmi $f(x_i)$, ampak s približki $\tilde{f}(x_i)$).
Recimo, da velja $|f(x_i) - \tilde{f}(x_i)| \leq \epsilon$. Naj bo $F f = \sum_{i = 0}^{n} A_i f(x_i)$.
\begin{equation*}
    D_a = |\sum_{i=0}^{n} A_i (f(x_i) - \tilde{f}(x_i))| \leq \sum_{i=0}^{n} |A_i| \epsilon = \epsilon \sum_{i=0}^{n} |A_i|
\end{equation*}
\begin{equation*}
    \sum_{i=0}^{n} A_i * 1 = F1 = S1 = \int_{a}^{b} 1dx = b-a
\end{equation*}
(manjka en komentar)
Če so uteži vse pozitivne, potem $D_a \leq \epsilon(b-a)$ in ni težav, če $n$ večamo.
