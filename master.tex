...
% 6. 3. 2023
Vsak $p \in \Pp_n$ lahko zapišemo kot $\sum_{i = 0}^{n} c_i \prod_{j = 0}^{i-1} (x-x_j)$. Iščemo koeficiente $(c_i)_{i=0}^n$, da bo $p$ interpolacijski.
Ta $p$ bomo konstruirali $\textbf{rekurzivno}$.
Naj bo $p_{k-1}$ interpolacijski polinom za $f$ na točkah $x_0, x_1, \dots, x_{k-1}$. Kako poiskati $p_k$, ki bo interpolacijski za $f$ na točkah $x_0, x_1, \dots, x_k$?
$p_{k-1} \in \Pp_{k-1}, p_k \in Pp_k$
\begin{equation*}
...
\begin{equation*}
    p_k(x_k) = f(x_k)
\end{equation*}
(poglej, ce kaj manjka)


$c$ določimo tako, da je izpolnjen še ta pogoj.
Ta $c$ je $\textbf{vodilni koeficient}$ od $p_k$. Označimo ga z $[x_0, x_1, \dots, x_k] f$ in ga imenujemo $\textbf{deljena diferenca}$.
\begin{defn}
    Deljena diferenca  $[x_0, x_1, \dots, x_k] f$ je vodilni koeficient interpolacijskega polinoma stopnje $k$ (keoficient pri $x^k$) za funkcijo $f$ na
...
\begin{align*}
    p_n(x) =& p_{n-1}(x) + [x_0, x_1, \dots, x_n] f (x-x_0)(x-x_1) \dots (x-x_{n-1}) \\
           =& \dots \\
           =& p_o(x) + [x_0, x_1] f (x-x_o) + [x_0, x_1, x_2] f (x-x_0)(x-x_1) + \dots + [x_0, x_1, \dots, x_n] f (x-x_0)(x-x_1) \dots (x-x_{n-1}) \\
    p_n(x) = & \sum_{i = 0}^{n}[x_0, x_1, \dots, x_i] f (x-x_0)(x-x_1)\dots(x-x_{i-1})
\end{align*}
Temu rečemo Newtonova oblika zapisa interpolacijskega polinoma.
...
\begin{theorem}[Rekurzivna formula za deljene diference]
    ...
    \begin{equation*}
        [x_0, x_1, \dots, x_k]f = \frac{[x_0, x_1, \dots x_k] f - [x_0, x_1, \dots x_{k-1}]f}{x_k - x_0}
    \end{equation*}
\end{theorem}

\begin{proof}
    Naj bo $q_0$ interpolacijski za $f$ na točkah $x_0, x_1, \dots, x_{k-1}$ in $q_1$ interpolacijski za $f$ na točkah $x_0, x_1, \dots, x_k$.
    Velja, da sta $q_0, q_1 \in \Pp_{k-1}$.
    Kako do polinoma $p$, ki bo interpolacijski na $x_0, x_1, \dots, x_k$? Ta $p$ bo $p \in \Pp_k$.
    \begin{equation*}
    ...
    Pogoj **
    \begin{equation*}
        p(x_j) = \ell_0(x_j) q_0 (x_j) + \ell_1 (x_j) q_1 (x_j) = (\ell_0 (x_j) + \ell_1 (x_j)) f(x_j) =? f(x_j)
    \end{equation*}
    Pogoja *
    \begin{equation*}
        p(x_0) = \ell_0(x_0) f(x_0) + \ell_1 (x_0) q(x_0) =? f(x_0)
    \end{equation*}
    \begin{equation*}
        p(x_k) = \ell_0 (x_k) q(x_k) + \ell_1(x_k) f(x_k) =? f(x_k)
    \end{equation*}
    Če izberemo
    \begin{equation*}
    ...
    \end{equation*}
    zadostimo pogojema *. Ker pa je $\ell_0(x) + \ell_1(x) = 1$ za $\forall x$, pa velja tudi **
    ...
    \begin{equation*}
        \text{vod.koef.}(p) = [x_0, x_1, \dots, x_k]f
    \end{equation*}
    Na desni strani:
    \begin{equation*}
        \frac{1}{x_0-x_k} \cdot vod.koef. (q_0) + \frac{1}{x_k - x_0} vod.koef. (q_1) = \frac{[x_1, x_2, ..., x_k] f - [x_0, x_1, ..., x_{k-1}] f}{x_k - x_o}
    \end{equation*}
\end{proof}
...
$n = 4$
\begin{align*}
    p(x) =& d_0 \cdot 1 + d_1 (x-x_0) + d_2 (x-x_0)(x-x_1) + d_3 (x-x_0)(x-x_1)(x-x_2) + d_4 (x-x_0)(x-x_1)(x-x_2)(x-x_3) \\
...
$v_n = d_n$
$for i = n-1:-1:0$
$   v_i = d_i + (x-x_i) v_{i+1}$
$end$
$v_0 = p(x)$

Algoritem(posplošen Hornerjev algoritem)
Vhod: $d_ (= [d_0, d_1, ..., d_n]), x_ = [x_0, x_1, ..., x_n], x$
$v_n = d_n$
$for i = n-1:-1:0$
$   v_i = d_i + (x-x_i)c_{i+1}$
$end$
Izhod: $v_0$
